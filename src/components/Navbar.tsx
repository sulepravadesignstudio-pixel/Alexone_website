...
export default function Navbar({ onNavigate, currentPage, onGoBack, onBack }: NavbarProps) {
      ...
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        >
          <img
            ...
            alt="Alexone"
            className={`object-contain transition-all duration-300 ${isScrolled ? 'h-12 md:h-14' : 'h-14 md:h-[4.8rem]'}`}
          />
        ...
        <div className="hidden md:flex flex-1 items-center justify-end gap-8">
          {currentPage !== 'home' && (
            <motion.button
              ...
              onClick={() => onBack?.()}
              className="inline-flex items-center gap-2 rounded-full border border-white/14 bg-black/15 px-4 py-2 text-[10px] uppercase tracking-[0.3em] text-white/82 transition-all duration-300 hover:border-[#C9A84C]/55 hover:bg-[#C9A84C]/14 hover:text-[#FFF3D0]"
            >
              <ArrowLeft size={13} />
              Back
            </motion.button>
            ...
            <div className="flex flex-col gap-0 px-6 py-5">
              ...
              {currentPage !== 'home' && (
                <button
                  ...
                  className="mt-4 rounded-full border border-white/14 bg-black/12 py-3 text-center text-[10px] uppercase tracking-[0.32em] text-white/86"
                >
                  ← Back
                </button>
              )}
