...
import { applyEntrySeo, applyRouteSeo, injectOrganizationJsonLd } from './seo/applySeo';

/** Routes that map to a screen; used for URL hash + History API so browser Back/Forward match in-app navigation. */
const VALID_PAGES = new Set([
  ...
  'cafe',
  'saloon',
  /** Unlisted — only via direct URL `#/privacy` */
  'privacy',
]);
...
function FloatingSidebar({
  ...
  return (
    ...
    >
      ...
      {/* Contact Us — vertical text (same form as Enquire modal) */}
      <button
        type="button"
      ...
      {/* Instagram */}
      <a
        href="https://www.instagram.com/alexone_interior"
        target="_blank"
        rel="noopener noreferrer"
      ...
      {/* Facebook */}
      <a
        href="https://www.facebook.com/alexoneinterior"
        target="_blank"
        rel="noopener noreferrer"
        className="group p-3 transition-all duration-300 hover:bg-[#C9A84C]/15"
        title="Facebook"
...
export default function App() {
  ...
  /** After the intro, sync initial route from hash and register one history entry per screen so Back/Forward work like a multi-page site. */
  useEffect(() => {
  ...
  /** Title, meta description, Open Graph, Twitter, and canonical per route */
  useEffect(() => {
    if (!hasEntered) {
      applyEntrySeo();
      return;
    }
    applyRouteSeo(currentPage);
  }, [hasEntered, currentPage]);

  useEffect(() => {
    if (!hasEntered) return;
    const onPopState = (e: PopStateEvent) => {
      const st = e.state as { page?: string } | null;
      const page =
        st && typeof st.page === 'string' && VALID_PAGES.has(st.page) ? st.page : getPageFromLocation();
      setCurrentPage(page);
      window.scrollTo(0, 0);
    };
        ...
        {!hasEntered ? (
          ...
          >
            ...
            <EnquiryModal
              ...
              onSuccess={() => handleNavigate('thank-you')}
            />
          </motion.div>
        )}
      </AnimatePresence>
